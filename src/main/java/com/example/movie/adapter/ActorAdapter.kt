...
class ActorAdapter : RecyclerView.Adapter<ActorAdapter.ElementViewAdapter>() {
    private var mValues = mutableListOf<Movie>()

    fun setData(movies: List<Movie>) {
        val filteredByYear= movies.filter { it.year == 2020 }
        val sortedMovies = filteredByYear.sortedByDescending { myObject -> myObject.rating}

        mValues.clear()
        mValues.addAll(sortedMovies)
